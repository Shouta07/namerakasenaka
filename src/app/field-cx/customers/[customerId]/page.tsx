...
import { CustomerActions } from "@/components/field-cx/customer-actions";
import { FIELD_CX_CUSTOMERS, KIND_META } from "@/lib/field-cx/fixtures";
import { cn } from "@/lib/utils/cn";

export function generateStaticParams() {
  return FIELD_CX_CUSTOMERS.map((c) => ({ customerId: c.id }));
}
...
export default async function FieldCxCustomerDetailPage({
  params,
}: {
  params: Promise<{ customerId: string }>;
}) {
  const { customerId } = await params;
  const customer = FIELD_CX_CUSTOMERS.find((c) => c.id === customerId);
