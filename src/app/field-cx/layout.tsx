...
import { FieldCxNav } from "@/components/field-cx/field-cx-nav";

export const metadata: Metadata = {
  title: {
    default: "Field CX ｜ 血液検査 × 肌改善の継続伴走 SaaS",
    template: "%s ｜ Field CX",
  },
  description:
...
export default function FieldCxLayout({
  children,
}: {
    ...
    <div className="min-h-[100dvh] bg-[#faf8f4]">
      ...
      <footer className="mx-auto max-w-6xl px-5 pb-8 sm:px-8">
        <p className="flex flex-wrap items-center gap-x-3 border-t border-stone-200 pt-4 text-[11px] text-stone-400">
          <span>
            Field CX — バイタリティデザイン合同会社 ｜ 本画面はデモです（操作はこの端末にのみ保存されます）。機能の増減と導入初日の表示は、右上の「デモ設定」から。
          </span>
          <a
            href="/hub"
