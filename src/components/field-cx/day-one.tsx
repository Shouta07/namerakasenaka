...
import { getDataset, setDataset, type FieldCxDataset } from "@/lib/field-cx/store";

/** 現在のデータ母集団。未取得のあいだは null（ちらつき防止）。 */
export function useDataset(): FieldCxDataset | null {
  const [dataset, setLocal] = useState<FieldCxDataset | null>(null);
  useEffect(() => {
...
export function DatasetSwitch() {
  ...
  function choose(next: FieldCxDataset) {
