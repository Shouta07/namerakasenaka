# ADR-001: hisrecoveries × Vitality Design プラットフォーム設計

**版**: v1.0（2026-07）／ ステータス: 提案 → ユーザー承認で「採択」に更新
**前提**: hisrecoveries は山本作の既存プロダクト（患者側・hisrecoveries.com/member）。
Vitality Design はその**配下**で開発する。最適化の優先軸 = **①セキュリティ ②データ保存 ③連携**。

---

## 決定（サマリ）

> **1つの背骨（共有 Identity + 共有 Postgres）に、2つのサーフェス（患者面 hisrecoveries / 院面 Vitality Design）。**
...
```
      ...
      ┌────────────────────────┬────────────────────────┐
      │  member.hisrecoveries  │  vitality-design.hisrecoveries   │   ← サーフェス分離（別app）
      │  患者面（B2B2C）        │  院面（B2B）             │      ※Vitality Designは将来 vitality-design.jp 等に
      ...
      │  schema vitality-design     … counseling / funnel / roleplay / copilot │
...
### 連携面 — 「最良の連携は、連携を無くすこと」
hisrecoveries と Vitality Design を別システムにして API/webhook で繋ぐと、二重書き込み・同期ズレ・
...
| 3 | **スキーマ×DBロール分離**（巻き添え半径） | `vitality_design_app` ロールは recoveries の実表に触れない（定義済みビュー/RPCのみ）。`recoveries_app` は読み取り中心。`service_role` はサーバ側ジョブ（ETL・集計・削除カスケード）専用でクライアントに絶対出さない |
...
| 保持・削除 | 解約後90日で削除。**患者単位の削除カスケード**（vitality-design側エビデンス・recoveries側・Storage写真を横断）を初日に実装。要配慮情報のため物理削除 |
...
| 連携 | 形 |
|---|---|
| hisrecoveries ⇄ Vitality Design | **連携しない（同一DBの射影）**。患者面は `recoveries` スキーマのビュー経由で読む |
...
- 別DB2本 + 同期ジョブ（結果整合の泥沼）
- 患者面から Vitality Design 内部APIを呼ぶ（プリンシパル混線）
...
2. 共有スキーマ v1（shared/vitality-design/recoveries）+ RLS + ロール3種 + 監査
3. 患者面: 既存 /share・/c/guide を `recoveries` ビュー読みに載せ替え（見た目は不変）
4. 院面: Vitality Design デモを実データ化（既存 M0〜M2 計画どおり）
