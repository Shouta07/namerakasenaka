...
## 17. 回復ガイド追補（三社共同開発, v0.3 ドラフト）

### 17.1 三社体制

| 会社 | 役割 |
|---|---|
| **エクシアクリニック** | 検査（IgGフードアレルギー / 腸内カンジダ / リーキーガット等）の実施、検査結果と医師コメントの提供 |
| **なめらかせなか** | サロン運用・カウンセリング・顧客への伴走 |
| **バイタリティデザイン合同会社** | 本モジュールの開発・運営 |

### 17.2 プロダクト本質

検査結果の表示システム**ではない**。顧客の回復ジャーニーをデジタル化する：

> 理解する → 納得する → 今日やることが分かる → 少し実践する → 変化を感じる → 自分で整えられる自信がつく

すなわち「**顧客が自走できる回復体験**」。共有 URL（`/share/[token]`）をスマートフォンで開くだけで、ログインなしに自分専用のガイドと日々のチェックが完結する。

### 17.3 法的整理

- 本モジュールは**医療診断アプリではない**。エクシアクリニックの検査結果の「**翻訳・教育・伴走**」ツールという位置づけ。
- 医師コメントはクリニックが提供し、サロンは医療行為をしない。顧客向けフッタに「本ガイドは医療診断ではありません。検査・診断についてはエクシアクリニックにご相談ください。効果には個人差があります。」を常時表示する。
- AI 出力は §8.2 の禁止語フィルタ **+ 追加禁止表現（治ります / 必ず改善します / 病気です / 医師の指示は不要 / これをしないと悪化 等）** を通過必須。ヒットした場合は保存せずエラーとし、再生成を促す。

### 17.4 表現ルール

- 専門用語は**小学6年生レベル**にやさしく翻訳する（リーキーガット / 腸内カンジダ菌 / IgG / カゼイン / グルテン などは折りたたみ式「やさしい解説」で提供）。
- **断定しない**。「〜の可能性があります」「先生の方針に沿って」を使う。
- 不安を煽らない。顧客を責めない。色も警告的な赤を避け、淡いローズ／ペールグリーンのトーンで示す。
- **7割OK思想**：今週やることは必ず3つだけ。「7割できればOK。できない日があっても大丈夫です。」を明示し、完璧主義にさせない。
- 最後は前向きな一言で締める。

### 17.5 データ

- テーブル：`guide_customers`（発行先。既存 `clients` と任意リンク） / `health_records`（検査結果メモ・医師コメント・サロンメモ・AI生成 JSON） / `daily_checks`（1顧客1日1行のセルフチェック）。マイグレーション `0009_recovery_guides.sql`。
- スタッフ（salon_admin / therapist, 同一組織）のみ RLS で読み書き。super_admin は全件。
- 顧客閲覧は **share_token による認証なしアクセス**（URL を知る人のみ）。RLS に匿名ポリシーは置かず、サーバールートが service-role クライアントでトークンを照合する。チェック記録は `POST /api/daily-checks`（トークン検証後に upsert）。
- AI 生成は `claude-sonnet-4-6`。`POST /api/recovery-guide/generate`（本番・永続化）/ `POST /api/recovery-guide/preview`（無永続・デモ経路）。全テーブルに監査トリガ。

### 17.6 未決事項

- share URL の**有効期限ポリシー**（無期限か、再発行運用か）。
- クリニックからの**データ受領方法**（紙→サロンで手入力 or CSV 連携）。
- 三社間の**データ取扱い契約**（検査結果の保管責任・削除依頼フロー・再委託の整理）。
