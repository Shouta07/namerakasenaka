...
> **美容・ウェルネス店舗の現場CXを創る。**
> 店側の画面（Field CX）は「上達と継続」の道具、お客様側の画面は「歩み・お祝い・人の温度・次の一歩」の体験。
> 記録を溜めるためではなく、現場の顧客体験（CX）そのものを商品にするためのプロダクト群である。
> 実装例: `/c/progress` — 進捗の記録を「歩みの物語」（日数・折り返しのお祝い・実感スコアの推移・担当のひとこと・今日のひとつ）として届ける。

## 0. 事業の一番強い構造（最上位テーゼ）

> **SaaSが増えるほど、Field CXの価値は上がる。** 院長は「何を選び・どう組み合わせ・どう成果に変えるか」で溺れる。
> Field CX は個々のSaaSと戦わず、**その上に立つ設計者**として、L1サービス（現場AX）で入り、
> L2統合（既存SaaSの上に乗る）で居座り、L3プロダクト（横断で繰り返す機能の製品化）で稼ぐ。
> 詳細と成立条件・罠は `docs/field-cx/moat-and-flywheel.md`。以下 §1〜 はその L3/L2 の具体である。

## 1. プロダクト定義

> **Field CX は、クリニック・サロンの「初回カウンセリング」を、いちばん得意な接客に変える SaaS。**
> AI相手の接客練習 × 成約の見える化 × 月1回の伴走で、スタッフの接客を底上げする。
> 記録・写真の経過は本人同意のもと LINE でお客様の手元に届き、成約後の関わりも顧客ごとに蓄積される。
>
> **（v2）そして Field CX は、電子カルテ・予約システムと戦わない。それらの上に乗る
...
- **なめらかせなか（senacare）= 1社目の導入事例**。翻訳辞書・図解・伴走ループ・LINE共有の設計思想は Field CX の土台としてそのまま流用する。
- デモは稼働済み（`/field-cx` 4画面 + モジュール増減機構）。本書はこれを**マルチテナント SaaS に昇格させる**設計である。
...
- 初回予約は月 10 件以上ある（集客はできている＝Field CX の効果が数字に出る）
- **除外**: 集客に困っている店舗（Field CX は集客ツールではない。予約系SaaSと競合しない位置取り）

### 2.3 競合と差別化

| 分類 | 例 | 彼らの主戦場 | Field CX の違い |
|---|---|---|---|
...
Field CX のカテゴリは彼らと別物として立てる:

> **彼ら = 記録のシステム（system of record）。Field CX = 上達のシステム（system of enablement）。**
> 業務を記録・効率化するツールは揃った。しかし「**人の接客が上手くなる**」「**患者との関係の質が上がる**」
> ツールは存在しない。Field CX はそこにだけ立つ。
...
- B4A・キレイパス導入院は競合ではなく**見込みリスト**。「お使いの system はそのままで、
  上に Field CX を乗せる」が営業の第一声。data-import の最初の対応フォーマットは両社のCSVエクスポート
...
**一言の差別化（v2.1）**: 「記録するツールは、もうお持ちのはず。**Field CX はスタッフを上手くし、
患者さんとの関係を深くする方のツール**です。お使いのシステムの上にそのまま乗ります」

### 2.4 ペイン → 価値マップ（v2・設計の背骨）

自由診療・美容クリニックのペインを6領域に棚卸しし、Field CX の提供価値と既存資産に対応付ける。
新機能の追加判断は必ずこの表に載るかで行う。

| # | ペイン領域 | 代表ペイン | Field CX の価値 | 対応トッピング |
|---|---|---|---|---|
...
4. **コンテンツ/セミナー** — 「成約率の教科書」ウェビナー月1回。デモ（/field-cx）をその場で触らせる
...
- 現行デモ（senacare リポジトリ）の設計思想を継承しつつ、**Field CX は独立リポジトリ・独立 Supabase プロジェクト**に切り出す（テナントデータと1社目カスタム実装を混ぜない）
- デモモード機構（env 未設定時 localStorage）は営業デモ用に**維持**する — `/field-cx` デモがそのまま営業資産になる
...
| （システム）`field_cx_admin` | 運営用。伴走レポート閲覧、サポート impersonate（監査ログ必須） |
...
見せるだけのBIと違い、**気づき → 打ち手の提案 → Field CX内でワンタップ実行 → 効果測定**が
...
```sql
-- 日次メトリクス（コパイロットの燃料。Field CX内データ + data-import の外部データを集約）
metrics_daily(
...
- 外部データ（data-import）が無くても Field CX 内データだけで動く。連携は精度を上げる追加燃料という位置づけ
- 提案の実行は必ず人の承認を挟む（自動送信はしない）。実行ログは監査ログに記録

## 9. モジュール & エンタイトルメント設計

「機能を増減できる」を課金と直結させる。**モジュール定義（現行 `FIELD_CX_MODULES`）を DB 化**し、プラン → エンタイトルメントに写像する。
...
- **Stripe Billing / subscription**。Product: `field_cx_starter/standard/pro`（月次・年次 Price）、`theme_pack`（one-time）、`setup_fee`（one-time）
...
- **店舗ごとに LINE公式アカウントを店舗名義で開設**（Messaging API チャネル）。Field CX に channel secret / access token を登録（暗号化保存）。※Field CX 名義の単一チャネルにしない — 顧客から見た差出人は店舗であるべき + 従量課金を店舗負担にできる
- 友だち追加: 店頭 QR（Field CX が生成）→ `line_user_id` と `customers` を紐付け（初回カウンセリング時にスタッフが照合）
- **同意ファースト（現行デモの設計を昇格）**:
...
2. Field CX の商標・ドメイン取得（field-cx-app.jp 等）
