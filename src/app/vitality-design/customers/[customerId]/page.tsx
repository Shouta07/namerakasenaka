...
import { CustomerActions } from "@/components/vitality-design/customer-actions";
import { VITALITY_DESIGN_CUSTOMERS, KIND_META } from "@/lib/vitality-design/fixtures";
import { cn } from "@/lib/utils/cn";

export function generateStaticParams() {
  return VITALITY_DESIGN_CUSTOMERS.map((c) => ({ customerId: c.id }));
}
...
export default async function VitalityDesignCustomerDetailPage({
  params,
}: {
  params: Promise<{ customerId: string }>;
}) {
  const { customerId } = await params;
  const customer = VITALITY_DESIGN_CUSTOMERS.find((c) => c.id === customerId);
