...
/**
 * Field CX のデモ用 localStorage ストア。
 *
 * senacare 本体の demo/store.ts と同じ思想:
 * - SSR ガード（window が無ければ既定値を返す）
 * - 単一 namespace `field-cx-demo-v1`
 * - モジュールの増減（トグル）を永続化するのが最重要の責務
 */

import {
  DEFAULT_MODULE_STATE,
  type FieldCxModuleId,
  type RubricKey,
} from "./fixtures";

const NS_KEY = "field-cx-demo-v1";
 ...
 */
export type FieldCxDataset = "full" | "dayone";

type FieldCxBlob = {
  dataset: FieldCxDataset;
  modules: Record<FieldCxModuleId, boolean>;
...
function read(): FieldCxBlob {
  if (typeof window === "undefined") return EMPTY;
  try {
    const raw = window.localStorage.getItem(NS_KEY);
    if (!raw) return EMPTY;
    const parsed = JSON.parse(raw) as Partial<FieldCxBlob>;
    return {
...
function write(blob: FieldCxBlob) {
  if (typeof window === "undefined") return;
  window.localStorage.setItem(NS_KEY, JSON.stringify(blob));
  window.dispatchEvent(new Event("field-cx-store"));
}
...
export function getDataset(): FieldCxDataset {
  return read().dataset;
}

export function setDataset(dataset: FieldCxDataset) {
  const blob = read();
...
export function getModuleState(): Record<FieldCxModuleId, boolean> {
  return read().modules;
}

export function setModuleEnabled(id: FieldCxModuleId, enabled: boolean) {
  const blob = read();
