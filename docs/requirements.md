...
| 会社 | 役割 |
|---|---|
| **提携クリニック** | 検査（IgGフードアレルギー / 腸内カンジダ / リーキーガット等）の実施、検査結果と医師コメントの提供 |
...
- 本モジュールは**医療診断アプリではない**。提携クリニックの検査結果の「**翻訳・教育・伴走**」ツールという位置づけ。
- 医師コメントはクリニックが提供し、サロンは医療行為をしない。顧客向けフッタに「本ガイドは医療診断ではありません。検査・診断については提携クリニックにご相談ください。効果には個人差があります。」を常時表示する。
