...
import { VITALITY_DESIGN_CUSTOMERS } from "@/lib/vitality-design/fixtures";
import {
  CUSTOMER_STATUSES,
  filterCustomers,
  type CustomerStatus,
} from "@/lib/vitality-design/filter";
...
export function CustomerList() {
  ...
  const results = useMemo(
    () => filterCustomers(VITALITY_DESIGN_CUSTOMERS, query, status),
    [query, status],
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-[12px] text-stone-500" aria-live="polite">
        {results.length} 名
        {filtering ? `（全 ${VITALITY_DESIGN_CUSTOMERS.length} 名中）` : ""}
      </p>
      ...
      {results.length === 0 ? (
        ...
        <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {results.map((c) => (
            <Link
              key={c.id}
              href={`/vitality-design/customers/${c.id}`}
