/**
 * Vitality Design — クリニック・サロンの初回カウンセリング支援サービス。
 *
 * デモ用フィクスチャ。Vitality Design は「機能モジュールを増減できる」ことが
 * コンセプトなので、MODULES を単一の情報源として、ナビ・トグル・
 * 各ページのガードすべてがここを参照する。
 */

export type VitalityDesignModuleId =
  | "counseling"
  | "labtest"
  | "copilot"
  | "roleplay"
...
export type VitalityDesignModule = {
  id: VitalityDesignModuleId;
  name: string;
  short: string;
  description: string;
  emoji: string;
...
export const VITALITY_DESIGN_MODULES: VitalityDesignModule[] = [
  {
    id: "counseling",
    name: "初回カウンセリング記録",
    short: "カウンセリング",
    ...
    emoji: "📋",
    core: true,
    href: "/vitality-design/customers",
  },
  {
    id: "labtest",
    name: "血液検査の翻訳ガイド",
 ...
 * 何を外し、なぜ外し、いつ戻すかは lib/vitality-design/mvp.ts に書いてある。
 * 商談で見せたいものは、デモ設定から一時的にオンにできる。
 */
export const DEFAULT_MODULE_STATE: Record<VitalityDesignModuleId, boolean> = {
  counseling: true,
  labtest: true,
  copilot: false,
  roleplay: false,
...
export type VitalityDesignCustomer = {
  id: string;
  name: string;
  age: number;
  status: "契約中" | "提案中" | "体験予約";
...
export const VITALITY_DESIGN_CUSTOMERS: VitalityDesignCustomer[] = [
  {
    id: "suzuki",
    name: "鈴木 さやか",
    age: 32,
