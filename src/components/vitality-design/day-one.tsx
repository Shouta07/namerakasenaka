...
import { getDataset, setDataset, type VitalityDesignDataset } from "@/lib/vitality-design/store";

/** 現在のデータ母集団。未取得のあいだは null（ちらつき防止）。 */
export function useDataset(): VitalityDesignDataset | null {
  const [dataset, setLocal] = useState<VitalityDesignDataset | null>(null);
  useEffect(() => {
...
export function DatasetSwitch() {
  ...
  function choose(next: VitalityDesignDataset) {
