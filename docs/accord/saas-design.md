...
> **v2 変更点（経営コパイロットへの進化）**: ポジショニングを「初回カウンセリング支援ツール」から
> 「**既存システムの上に乗るクリニック経営のAIレイヤー**」に更新（§1・§2.4）。
> 新コア機能「経営コパイロット」を追加（§8B）。予約・カルテは引き続き作らないが、
> **連携はする**（`data-import`）方針に発展。トッピング4種追加は `topping-architecture.md` v2 を参照。
...
>
> **（v2）そして Accord は、電子カルテ・予約システムと戦わない。それらの上に乗る
...
自由診療・美容クリニックのペインを6領域に棚卸しし、Accord の提供価値と既存資産に対応付ける。
新機能の追加判断は必ずこの表に載るかで行う。

| # | ペイン領域 | 代表ペイン | Accord の価値 | 対応トッピング |
|---|---|---|---|---|
...
見せるだけのBIと違い、**気づき → 打ち手の提案 → Accord内でワンタップ実行 → 効果測定**が
...
```sql
-- 日次メトリクス（コパイロットの燃料。Accord内データ + data-import の外部データを集約）
metrics_daily(
...
- 外部データ（data-import）が無くても Accord 内データだけで動く。連携は精度を上げる追加燃料という位置づけ
