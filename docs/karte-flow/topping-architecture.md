...
**目的**: 「機能をトッピングできる SaaS」として Field CX をローンチするための、現状機能の棚卸しとモジュール設計
**前提**: `docs/field-cx/saas-design.md`（事業・技術設計 v1.0）の実装詳細版。矛盾時は本書が優先
...
### 1.1 Field CX デモ（`/field-cx` — 4画面・モジュール機構つき）

| # | 機能 | 実装 | SaaS昇格 |
|---|---|---|---|
| A1 | モジュール増減パネル + ModuleGate | `components/field-cx/module-panel.tsx`, `lib/field-cx/store.ts`（localStorage） | ◎ **トッピング機構の原型**。DB化して昇格 |
| A2 | AI接客練習（3ペルソナ・5観点採点・履歴） | `components/field-cx/roleplay-*.tsx`（台本エンジン） | ◎ Claude API 接続で本実装 |
| A3 | 成約ダッシュボード（ファネル/推移/スタッフ別/月1伴走） | `components/field-cx/charts.tsx`, `app/field-cx/dashboard` | ◎ 実データ接続 |
| A4 | 顧客フォロー（タイムライン/LINE同意ファースト送信/メモ） | `app/field-cx/customers`, `customer-actions.tsx` | ◎ コア（生地）に昇格 |
...
| 基盤 | 実装 | 備考 |
|---|---|---|
| フラグ機構（現状3系統に分裂） | ①`lib/field-cx/store.ts` の UI トグル ②`lib/billing/feature-flags.ts`（env + 顧客別フラグ） ③ロール別ルートグループ | **本書で1系統に統一する**（§3） |
...
現行 `FIELD_CX_MODULES`（fixtures.ts）を拡張し、**コードのレジストリ + DB のエンタイトルメント**の2段構成にする。トッピングの「定義」はコード（型安全・レビュー可能）、「誰が使えるか」は DB。
...
| 層 | 実装 | 現行資産 |
|---|---|---|
| ナビ | レジストリの `nav` から動的生成。オフなら消える | `FieldCxNav` の一般化 |
...
### 3.5 移植マップ（senacare → Field CX SaaS）
...
| Field CX 4画面 | デモ→本実装（saas-design M0〜M4 の通り） | 主工数 |
...
- テナントごとのコードカスタマイズ — 差異はすべて entitlement とコンテンツパックで表現する（1社目=なめらかせなか専用実装を Field CX 本体に持ち込まない）

---

## 5. 次のアクション

1. **レジストリ実装**（M0 内）: `lib/toppings/` に registry / can() / ToppingGate。現行 `FIELD_CX_MODULES` からの移行
2. **entitlements テーブル + Stripe 連携**（M2）: plan→トッピング束の写像、addon 購入フロー
3. **プラン×トッピング表の営業資材化**: /plans または新 /field-cx/pricing にトッピング表を載せる（「機能をトッピングできる」はそれ自体が営業トークになる）
