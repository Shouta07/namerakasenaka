...
> **美容・ウェルネス店舗の現場CXを創る。**
> 店側の画面（Accord）は「上達と継続」の道具、お客様側の画面は「歩み・お祝い・人の温度・次の一歩」の体験。
> 記録を溜めるためではなく、現場の顧客体験（CX）そのものを商品にするためのプロダクト群である。
> 実装例: `/c/progress` — 進捗の記録を「歩みの物語」（日数・折り返しのお祝い・実感スコアの推移・担当のひとこと・今日のひとつ）として届ける。

## 0. 事業の一番強い構造（最上位テーゼ）

> **SaaSが増えるほど、Accordの価値は上がる。** 院長は「何を選び・どう組み合わせ・どう成果に変えるか」で溺れる。
> Accord は個々のSaaSと戦わず、**その上に立つ設計者**として、L1サービス（現場AX）で入り、
> L2統合（既存SaaSの上に乗る）で居座り、L3プロダクト（横断で繰り返す機能の製品化）で稼ぐ。
> 詳細と成立条件・罠は `docs/accord/moat-and-flywheel.md`。以下 §1〜 はその L3/L2 の具体である。
...
Accord のカテゴリは彼らと別物として立てる:

> **彼ら = 記録のシステム（system of record）。Accord = 上達のシステム（system of enablement）。**
> 業務を記録・効率化するツールは揃った。しかし「**人の接客が上手くなる**」「**患者との関係の質が上がる**」
> ツールは存在しない。Accord はそこにだけ立つ。
...
- B4A・キレイパス導入院は競合ではなく**見込みリスト**。「お使いの system はそのままで、
  上に Accord を乗せる」が営業の第一声。data-import の最初の対応フォーマットは両社のCSVエクスポート

**山本にしかできないこと（機能ではなく方法論・関係資産＝コピーされにくい）**:
1. **翻訳者ポジション** — 真弓先生（医療知見）× 現場 × 顧客語の翻訳パイプライン。翻訳辞書・
...
**一言の差別化（v2.1）**: 「記録するツールは、もうお持ちのはず。**Accord はスタッフを上手くし、
