...
import { VITALITY_DESIGN_MODULES, type VitalityDesignModuleId } from "@/lib/vitality-design/fixtures";
import { getModuleState, setModuleEnabled } from "@/lib/vitality-design/store";

/**
 * 機能モジュールの増減パネル。
 * Vitality Design のコンセプト「必要な機能だけ、必要なときに」をそのまま UI にする。
 */
/**
...
export function ModulePanel({ compact = false }: { compact?: boolean } = {}) {
  ...
  function toggle(id: VitalityDesignModuleId, name: string, next: boolean) {
...
export function ModuleGate({
  ...
  const meta = VITALITY_DESIGN_MODULES.find((m) => m.id === module);
  return (
    <div className="mx-auto max-w-xl rounded-3xl border border-dashed border-stone-300 bg-white p-10 text-center">
      ...
      <p className="mt-2 text-sm leading-relaxed text-stone-600">
        Vitality Design は必要な機能だけを選んで使えます。このモジュールを使う場合は、概要ページからオンにしてください。
      </p>
